...
public class SwiftVideoCallPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    var window: UIWindow?
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = SwiftVideocallPlugin(uiViewController: viewController)

        registrar.addMethodCallDelegate(instance, channel: channel)

        // Set up event channel for conference events
        let eventChannelName = "video_call_events"
        ...
        if (call.method == "joinMeeting") {
            ...
            if let myArgs = args as? [String: Any]
            {
                if let roomName = myArgs["room"] as? String {
                    ...
                    if let configOverrides = myArgs["configOverrides"] as? Dictionary<String, Any>
                    {
